...
本文方法的总体框架如图1所示。给定源代码函数样本，首先通过代码预处理和图结构构建得到 XFG 样本，并输入已训练漏洞检测模型，得到样本漏洞预测概率。随后，从模型前向传播过程中提取节点级或语句级注意力证据，并依据节点与源代码行号之间的映射关系聚合为行级 Attention Score。与此同时，方法根据代码行文本计算 Semantic Risk Score，用于刻画危险 API、内存操作、外部输入、数组指针访问和边界相关操作等安全语义风险。最后，将两类分数归一化后进行线性融合，得到每个候选代码行的综合漏洞定位分数，并按照分数降序输出 Top-k 候选漏洞行。
...
flowchart TD
    ...
    C --> E["注意力证据提取"]
    B --> F["源代码行文本"]
    E --> G["行级 Attention Score"]
    F --> H["Semantic Risk Score"]
    G --> I["归一化与证据融合"]
    H --> I
    I --> J["候选漏洞行排序"]
    J --> K["Top-k 漏洞行与解释输出"]
...
与端到端重新训练的行级检测模型不同，本文方法强调对已有漏洞检测结果的解释和细粒度定位增强。模型漏洞预测概率用于判断样本整体风险，综合评分则用于回答“哪些代码行更可能与漏洞相关”。因此，本文方法既保留原模型的检测能力，又补充面向人工审计的行级解释结果。
...
本文采用最大池化而非平均池化，是因为漏洞行通常由其中最关键的危险节点决定。如果同一行包含多个节点，平均池化可能削弱关键节点贡献，而最大池化能够保留最强证据。随后，对一个样本内所有候选行的注意力分数进行 min-max 归一化，得到 $\hat{A}(v_i)$，用于后续融合。
...
为综合模型内部证据和安全语义先验，本文采用线性融合方式构建综合漏洞定位分数。对于候选代码行 $v_i$，综合评分定义为：
...
图2展示了注意力证据与语义风险分数的融合流程。该流程先分别归一化两类分数，再根据权重 $\lambda$ 计算 FinalScore，最后按照分数降序生成候选漏洞行列表。
...
图2说明：注意力证据反映模型内部关注区域，语义风险分数反映代码行的安全敏感语义。两类分数经过归一化后进行线性融合，得到用于主排序的综合定位分数。
...
在解释输出中，每条候选行包含行号、代码文本、注意力证据分数、语义风险分数、综合分数和语义标签。该设计使输出结果既能用于定量评价，也能辅助人工审计。对于具有真实行级标签的正样本，可计算 Top-k Accuracy、MRR 和 IFA 等指标；对于实际审计场景，开发者可以优先检查排名靠前的候选行，并结合语义标签理解该行被判定为高风险的原因。
...
3. 根据节点与源代码行号的映射关系，将节点证据聚合为行级 Attention Score；
4. 对每一条代码行 v_i：
...
8. 输出 Top-k 行号、代码文本、注意力分数、语义风险分数、综合分数和语义标签；
