...
DiverseVul-subset 是从原始 DiverseVul 数据集中筛选构造的实验子集，而不是完整原始数据集。核对后，本文从 DiverseVul 数据集中筛选得到 1513 个函数级样本，构建 DiverseVul-subset 数据集。其中，漏洞样本 487 个，非漏洞样本 1026 个。原始 DiverseVul 来源于真实开源项目漏洞修复提交，主要提供函数级漏洞标签；为开展行级定位泛化实验，本文采用补丁差异构造近似行级标签。具体而言，本文根据漏洞修复提交提取修复前后 diff，将被删除或被修改的代码行映射回漏洞函数内部，作为近似漏洞行标签；同时过滤补丁过大、修改范围过宽、无法稳定映射以及 tangled patch 严重的样本。因此，DiverseVul-subset 的样本来源真实，但其行级标签属于基于 patch diff 的近似标注，主要用于验证本文方法在真实项目漏洞场景下的泛化趋势。
